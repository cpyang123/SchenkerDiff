...
class Direction(object):
  ...
  def _parse_octave_shift(self, xml_shift):
    ...
    """
    default_size = "8"  # Default to one octave if size is not specified
    shift_type = xml_shift.attrib.get('type', 'unknown')  # Use 'unknown' or another sensible default
    shift_size = xml_shift.attrib.get('size', default_size)  # Use the default size if 'size' is not present

    self.type = {'type': 'octave-shift', 'content': shift_type, 'size':  shift_size}
